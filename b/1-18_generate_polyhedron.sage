# Notation: "!!!" means that this part should be adapted according the case
#! put the path here
...
def eps_polyhedron(eps):
    ...
    eqs9D = [ [e[0]] + [0]*len(vp_vars) + list(e[1:]) for e in eqs]
...
def Vm_polyhedron(q):
    ...
    qq = lambda x : RIF(x).simplest_rational() # upper instead of lower !!!
    
    exact_coronas_num = 0
    Rifr = PolynomialRing(RIF,len(vp_vars),names=vp_vars)
    ineqs = []
    l = list(false_corona_generator(q)) 
    ...
    for triangles in l:
        ...
        if diff.contains_zero():
            ...
            lbp = Rifr(lb)
            # mq >= lbp -> -lbp+mq >= 0
            ineq = list(map(qq, [-lbp.constant_coefficient()]+[-lbp.coefficient({y:1}) for y in vp_vars]+m_q_vector))
            ineqs+=[ineq]
...
def get_one_polyhedron(q, case):
    ...
    try:        
        ...
        print(str(case)+"  P"+q+" was created: "+str(Pq))
