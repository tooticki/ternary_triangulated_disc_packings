...
def curves_for_paper(ra,rb,rc):
    ...
    Ep = line([[x, E(rb+rc,x,ra+rb,ra,rb,rc)] for x in seg(bmin,bmax,100)],color=(0,0,0.5), thickness = .6)
    Up = line([[x, Unoz(rb+rc,x,ra+rb,ra,rb,rc)] for x in seg(bmin,bmax,100)],color='red', linestyle='dotted', thickness = .8)
    ...
    Dp=line([[x, dist(x,rb+rc,ra+rb,rb,ra,rc,radius(rb+rc,x,ra+rb,ra,rb,rc))] for x in seg(tD-QQ(.05),tD+QQ(.05),50)],color=(0,.7,.4),thickness = .5)
   ...
   # Zp = plot(z(rb), color = 'red', xmin=ra+rc, xmax=bmax, thickness = .8)
    ...
    #L =  line([(LQ[(ra,rc)][0],-0.01), (LQ[(ra,rc)][0],0.01)], color = "green", thickness = 3)
    
    L=text("\u2716", (LQ[(ra,rc)][0],0), color=(0,.7,.4),  fontsize=10, vertical_alignment='center')
...
#    L = line([(LQ[(ra,rc)][0],QQ(-0.03)), (LQ[(ra,rc)][0],QQ(0.03))], color = "green", thickness = 1)
    text("\u2716", (LQ[(ra,rc)][0],0), color='green',  fontsize=3)
...
def save_curves_for_paper(name=""):
    rttriangles = tripl(I)+tripl(r)+tripl(s)
    for (ra,rb,rc) in rttriangles:
        (curves_for_paper(ra,rb,rc)).save(dpi=500, filename = "curves/paper_"+str(CASE)+"_"+r_name(ra)+r_name(rb)+r_name(rc)+name+".pdf") 
